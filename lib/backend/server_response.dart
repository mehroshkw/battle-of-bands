...
class AddSongResponse extends StatusMessageResponse {
  ...
  factory AddSongResponse.fromJson(Map<String, dynamic> json) {
    final statusMessageResponse = StatusMessageResponse.fromJson(json);
    final songsJson = json.containsKey('data') ? json['data'] as Map<String, dynamic>? : null;
...
class LoginResponse {
  ...
  LoginResponse({required this.id, required this.imagePath, required this.name, required this.dateOfBirth, required this.emailAddress});
  ...
  LoginResponse.empty() : this(id: 0,
      imagePath: '',
      name: '',
      dateOfBirth: '',
      emailAddress: '');

  LoginResponse copyWith({String? imagePath, String? name, String? dateOfBirth, String? emailAddress}) =>
      LoginResponse(
          id: id,
  ...
  Map<String, dynamic> toJson() => {'id': id, 'imagePath': imagePath, 'fullName': name, 'dob': dateOfBirth, 'email': emailAddress};
}
...
class Statistics {
  ...
  final int judgedBattles;

  Statistics({required this.totalUploads, required this.totalWins, required this.totalBattles, required this.totalLoses, required this.judgedBattles});
  ...
  factory Statistics.fromJson(Map<String, dynamic> json) {
    ...
    final int judgedBattles = json.containsKey('judgedBattles') ? json['judgedBattles'] : 0;
    return Statistics(totalBattles: totalBattles,
        totalLoses: totalLoses,
        totalUploads: totalUploads,
        totalWins: totalWins,
        judgedBattles: judgedBattles);
  }

  Statistics copyWith({int?judgedBattles}) =>
      Statistics(totalUploads: totalUploads,
          totalWins: totalWins,
          totalBattles: totalBattles,
          totalLoses: totalLoses,
          judgedBattles: judgedBattles ?? this.judgedBattles);
}
...
class Song {
  ...
  factory Song.fromJson(Map<String, dynamic> json) {
    ...
    final num duration = json.containsKey('duration') ? json['duration'] ?? 0.0 : 0.0;
  ...
  Song copyWith({int? votesCount, bool? isVoted, Duration? seekbar}) =>
      Song(
          id: id,
          fileUrl: fileUrl,
          user: user,
  ...
  Map<String, dynamic> toJson() =>
      {'id': id, 'title': title, 'date': date, 'votesCount': votesCount, 'fileUrl': fileUrl, 'bandName': bandName, 'appUser': user, 'duration': duration};
}
