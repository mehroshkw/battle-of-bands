...
class SharedWebService {
  ...
  Future<LoginAuthenticationResponse> signup(
    ...
    final response = await _post(Uri.parse('$BASE_URL/Account/SignUp'),
        {'fullName': name, 'email': email, 'password': password, 'dob': dob});
  ...
  Future<IBaseResponse> changePassword(
    ...
    final response =
        await _post(Uri.parse('$BASE_URL/Account/ChangePassword'), {
  ...
  Future<Statistics> getStatistics(int userId) async {
    final uri =
        Uri.parse('$BASE_URL/Dashboard/GetStatistics?appUserId=$userId');
  ...
  Future<List<Song>> getLeaderboard(int genreId, int userId) async {
    final uri = Uri.parse(
        '$BASE_URL/Dashboard/GetLeaderboard?appUserId=$userId&genreId=$genreId');
    final response = await _get(uri);
    final responseBody =
        json.decode(await response.transform(utf8.decoder).join());
    return (responseBody as List<dynamic>)
  ...
  Future<List<Song>> getAllMySongs(int genreId, int userId) async {
    final uri = Uri.parse(
        '$BASE_URL/Song/GetAllMySongs?appUserId=$userId&genreId=$genreId');
    final response = await _get(uri);
    final responseBody =
        json.decode(await response.transform(utf8.decoder).join());
    return (responseBody as List<dynamic>)
  ...
  Future<List<Song>> getAllSongs(int genreId, int userId) async {
    final uri = Uri.parse(
        '$BASE_URL/Song/GetAllSongByGenreId?appUserId=$userId&genreId=$genreId');
    final response = await _get(uri);
    final responseBody =
        json.decode(await response.transform(utf8.decoder).join());
    return (responseBody as List<dynamic>)
  ...
  Future<void> voteSong(
      int songId, int userId, bool isVoted, int losserSongId) async {
    final response = await _post(Uri.parse('$BASE_URL/Song/Vote'), {
      "songId": songId,
      "appUserId": userId,
      "isVoted": isVoted,
