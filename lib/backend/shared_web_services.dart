...
class SharedWebService {
  ...
  Future<LoginAuthenticationResponse> signup(String name, String email, String password, String dob) async {
    final response = await _post(Uri.parse('$BASE_URL/Account/SignUp'), {'fullName': name, 'email': email, 'password': password, 'dob': dob});
  ...
  Future<void> voteSong(int songId, int userId, bool isVoted, int losserSongId) async {
    await _post(Uri.parse('$BASE_URL/Song/Vote'), {"songId": songId, "appUserId": userId, "isVoted": isVoted, "losserSongId": losserSongId});
  }
